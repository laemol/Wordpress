...
?><!DOCTYPE html>
<html <?php language_attributes(); ?> <?php if (isset($grandtour_homepage_style) && !empty($grandtour_homepage_style)) {
    echo 'data-style="' . esc_attr($grandtour_homepage_style) . '"';
...
<body <?php body_class(); ?>>
	...
	<?php
       ...
       if (!empty($pp_page_bg) && basename($pp_page_bg) == 'default.png') {
        ...
        } elseif (is_single() && empty($pp_page_bg) && !grandtour_is_woocommerce_page()) {
            $page_menu_transparent = 0;
        }
    ...
    ?>
	<div id="wrapper" class="<?php if (!empty($grandtour_page_menu_transparent)) {
        ?>hasbg<?php
    } ?> <?php if (!empty($page_menu_transparent)) {
        ?>transparent<?php
    } ?>">
