...
<form id="tour_search_form" name="tour_search_form" method="get" action="<?php echo get_the_permalink($id); ?>">
    <div class="tour_search_wrapper">
    		...
    		<select id="month" name="month">
	    		...
	    		<?php
                    foreach ($available_months as $key => $available_month) {
                        ?>
			    	<option value="<?php echo esc_attr($key); ?>" <?php if (isset($_GET['month']) && $_GET['month'] == $key) {
                            ?>selected<?php
                        } ?>><?php echo esc_attr($available_month); ?></option>
			    <?php
                    } ?>
    		</select>
    		...
    		<select id="sort_by" name="sort_by">
	    		<?php
                    foreach ($sort_options as $key => $sort_option) {
                        ?>
			    	<option value="<?php echo esc_attr($key); ?>" <?php if (isset($_GET['sort_by']) && $_GET['sort_by'] == $key) {
                            ?>selected<?php
                        } ?>><?php echo esc_attr($sort_option); ?></option>
			    <?php
                    } ?>
    		</select>
        ...
        <div class="one_fourth themeborder">
    		<input id="keyword" name="keyword" type="text" autocomplete="off" placeholder="Product" <?php if (isset($_GET['keyword'])) {
                        ?>value="<?php echo esc_attr($_GET['keyword']); ?>"<?php
                    } ?>/>
    		<span class="ti-search"></span>
    		<div id="autocomplete" class="autocomplete" data-mousedown="false"></div>
    	</div>
    	<div class="one_fourth last themeborder">
    		<input id="tour_search_btn" type="submit" class="button" value="ZOEKEN"/>
    	</div>
    	...
    	<div class="tour_advance_search_wrapper">
            ...
            if (!empty($tour_cats_arr)) {
                foreach ($tour_cats_arr as $key => $tour_cat) {
                    ?>
						<option value="<?php echo esc_attr($tour_cat->slug); ?>" <?php if (isset($_GET['tourcat']) && $_GET['tourcat'] == $tour_cat->slug) {
                        ?>selected<?php
                    } ?>><?php echo esc_attr($tour_cat->name); ?></option>
					<?php
                }
            } ?>
            ...
            if (!empty($destination_arr)) {
                foreach ($destination_arr as $key => $destination) {
                    ?>
						<option value="<?php echo esc_attr($destination->ID); ?>" <?php if (isset($_GET['destination_id']) && $_GET['destination_id'] == $destination->ID) {
                        ?>selected<?php
                    } ?>><?php echo esc_attr($destination->post_title); ?></option>
					<?php
                }
            } ?>
		    	</select><span class="ti-angle-down"></span>
	    	...
	    	<div class="one_fourth themeborder">
		    	<input id="budget" name="budget" type="text" placeholder="<?php esc_html_e('Max budget ex. 500', 'grandtour'); ?>" <?php if (isset($_GET['budget'])) {
                ?>value="<?php echo esc_attr($_GET['budget']); ?>"<?php
            } ?>/>
		    	<?php
